...
assign_region <- function(lat, lon) {
  ...
  } else if (lat > 23.5 & ((lon >= 260 & lon <= 360) | (lon >= 0 & lon <= 20))) {
    # North Atlantic: lat > 23.5°, lon between 260°-360° or 0°-20°
    return("North Atlantic")
  } else if (lat > 23.5 & lon >= 120 & lon < 260) {
...
total_counts <- df_argo_clean %>%
  group_by(lon_bin, lat_bin, local_season, region) %>%
...
# Aggregate counts by region and season
region_season_summary <- merged_counts %>%
  group_by(region, local_season) %>%
  summarize(
    total_anomalies = sum(count_anomaly),
    total_profiles = sum(count_total),
    proportion = total_anomalies / total_profiles,
    .groups = 'drop'
  )
