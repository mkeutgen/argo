...
for (j in seq_along(wmolist)) {
  try({
    ...
    if (length(list.plots) > 0) {
      dir <- paste0("~/Documents/ARGO/EddySubductionFigures196/", wmo)
      if (!dir.exists(dir)) {
...
detected.events.df %>% ungroup() %>% select(CYCLE_NUMBER,WMO) %>% unique()



write_csv(detected.events.df, "~/Documents/GLOBARGO/data/detected_events_1_96cutoff.csv")
