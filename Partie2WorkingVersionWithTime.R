...
for (j in seq_along(wmolist)) {
  try({
    ...
    # data_df <- data_df %>% filter(CYCLE_NUMBER %in% cycle_number)
...
detected.events.df <- detected.events.df %>% ungroup() %>% select(CYCLE_NUMBER, WMO) %>% unique()
write_csv(detected.events.df, "~/Documents/ARGO/ProcessedData/detected_events_unique.csv")
