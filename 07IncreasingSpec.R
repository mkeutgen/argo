...
for (j in seq_along(wmolist)) {
  ...
  # wmo <- 1902593
    ...
    if (length(list.plots) > 0) {
      dir <- paste0("/data/GLOBARGO/figures/EddySubductionFiguresSensitivityVarIncrease/", wmo)
      if (!dir.exists(dir)) {
...
# 3513 events detected
detected.events.df %>% dplyr::select(CYCLE_NUMBER,WMO) %>% unique()



write_csv(detected.events.df, "/data/GLOBARGO/data/detected_events_spec_incr.csv")
