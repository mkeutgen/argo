...
# should be only the variance of the residuals
# below some depth level under which we believe there might be a subduction
...
# Functions for ABS_SAL anomaly 
find_closest_to_surface <- function(data) {
  data %>%
    filter(!is.na(ABS_SAL)) %>%    # Filter out rows where ABS_SAL is NA
...
compute_mean_ABS_SAL_50m <- function(ABS_SAL_df) {
  ...
  filtered_df <- ABS_SAL_df %>%
    filter(PRES_ADJUSTED <= 50)
  
  # Calculate the mean of ABS_SAL, omitting missing values (NA)
  mean_ABS_SAL <- mean(filtered_df$ABS_SAL, na.rm = TRUE)
...
mean_ABS_SAL_at_min_max_levels <- function(data, target_pressure) {
  ...
  ABS_SAL_values <- filtered_data %>%
    ...
    pull(ABS_SAL)  # Extract ABS_SAL values
  
  # Calculate the mean ABS_SAL
...
for (j in seq_along(wmolist)) {
  try({
    ...
    data_df <- data_df %>% filter(!is.na(DOXY)) %>% group_by(CYCLE_NUMBER) %>%
      mutate(ABS_SAL = gsw::gsw_SA_from_SP(SP = PSAL_ADJUSTED, p = PRES_ADJUSTED,
    ...
    downscale_data_fun_wo_out <- function(df, b = 20) {
      data <- df %>%
        dplyr::select(PRES_ADJUSTED, AOU, ABS_SAL, CYCLE_NUMBER, LONGITUDE, LATITUDE, TIME)
    ...
    A <- df %>%
      group_by(CYCLE_NUMBER) %>%
      group_modify(~ .x %>%
                     dplyr::select(PRES_ADJUSTED, AOU, ABS_SAL, LATITUDE, LONGITUDE, TIME) %>%
                     ...
                     mutate(
                       IQRN = IQR(ROB.RES.RAW[PRES_ADJUSTED > 200], na.rm = TRUE) / 1.349,
                       MEDIAN_RES = median(ROB.RES.RAW[ROB.RES.RAW != 0 & PRES_ADJUSTED > 200], na.rm = TRUE)
                     ) %>%
                     mutate(
    ...
    prop_zero <- B %>% dplyr::select(AOU, ABS_SAL, CYCLE_NUMBER, PRES_ADJUSTED,LATITUDE,LONGITUDE,TIME) %>%
      unique() %>%
    ...
    for (i in 1:nrow(carb_eddy.id)){
      ABS_SAL <- data_df %>% filter(CYCLE_NUMBER==carb_eddy.id$CYCLE_NUMBER[i]) %>% dplyr::select(ABS_SAL,PRES_ADJUSTED) %>% ungroup()
      
      mean_ABS_SAL_surf <- compute_mean_ABS_SAL_50m(ABS_SAL)
      ...
      # Find the value of ABS_SALiness at the pressure level closest to target pressure
      closest_ABS_SAL <- ABS_SAL %>%
        slice_min(abs(PRES_ADJUSTED - target_pressure), n = 1) %>%
        pull(ABS_SAL) 
      
      # If ABS_SAL at outlying level is closer to surface value than the mean ABS_SAL is of the surface value, anomaly is consistent
      const.vec[i] <- ifelse( 
        abs(closest_ABS_SAL- mean_ABS_SAL_surf)  < 
          abs(mean_ABS_SAL_at_min_max_levels(ABS_SAL,target_pressure = target_pressure)-
                mean_ABS_SAL_surf) ,
        1,0)
    }
    ...
    for (i in seq_along(carb_eddy.id$CYCLE_NUMBER) ) {
      ...
      data_ABS_SAL <- current_data %>% filter(VAR == "ABS_SAL")
      
      # Calculate derivatives for AOU and ABS_SAL
      data_aou_deriv <- calculate_derivative(data_aou)
      data_ABS_SAL_deriv <- calculate_derivative(data_ABS_SAL)
    ...
    for (i in seq_along(eddy_dataframe$CYCLE_NUMBER)) {
      ...
      df.ds <- df.ds %>% ungroup() %>% dplyr::select(AOU, ABS_SAL, PRES_ADJUSTED) %>%
    ...
    if (length(list.plots) > 0) {
      dir <- paste0("/data/GLOBARGO/figures/EddySubductionFiguresSalinityVar/", wmo)
      if (!dir.exists(dir)) {
...
write_csv(detected.events.df, "/data/GLOBARGO/data/detected_events_abs_sal_var.csv")
