...
downscale_data_fun <- function(df, b = 20, cutoff = 1.96) {
  ...
  downscaled_data <- data %>%
    group_by(bin) %>%
    mutate(across(-c(PRES_ADJUSTED, LATITUDE, LONGITUDE, CYCLE_NUMBER, TIME), 
                  ~mean(., na.rm = TRUE)))
...
for (j in seq_along(wmolist)) {
  try({
    ...
    current_eddy.l %>% filter(CYCLE_NUMBER == 8)
    ...
    if (length(list.plots) > 0) {
      dir <- paste0("/data/GLOBARGO/figures/EddySubductionFiguresSalinityVarV2/", wmo)
      if (!dir.exists(dir)) {
...
classified_df %>% filter(WMO == 5904844)
